...
export default function GovernmentInstitutionsMasterTable() {
...
useEffect(() => {
  ...
  fetch("http://localhost:3000/dashboard/chl-hubusers")
    ...
    .then((data) => {
      ...
      // Fetch districts and institution data in parallel
      Promise.all([
        fetch(`http://localhost:3000/dashboard/chl-districts-by-hub?hub_id=${currentUser.hub_id}`).then((res) => res.json()),
        fetch("http://localhost:3000/dashboard/government-institution-master").then((res) => res.json()),
      ]).then(([districts, data]) => {
        setDistrictOptions(districts);

        // Now filter because hub_id is known
  ...
  const columns = [
    ...
    { field: "name", headerName: "Institution Name", width: 500 },
  ...
  const handleAdd = () => {
    ...
    fetch("http://localhost:3000/dashboard/government-institution-master", {
      method: "POST",
    ...
    <DashboardLayout>
      ...
      <Box p={2}>
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
          ...
          >
            {userHubName
              ? `${userHubName.toUpperCase()} – GOVERNMENT INSTITUTION MASTER DATA`
              : "GOVERNMENT INSTITUTION MASTER DATA"}
          </Typography>
        ...
        <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="sm" fullWidth>
          <DialogTitle sx={{ fontFamily: "Nunito, sans-serif" }}>Add Government Institution</DialogTitle>
